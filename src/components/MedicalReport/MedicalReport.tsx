...
const MedicalReport: React.FC = () => {
  ...
  if (id) {
    breadcrumbItems.push({ path: `/usuarios/${id}/detalles`, label: `Nombre_Usuario` });
  }

  if (location.pathname.includes("viewReport")) {
    breadcrumbItems.push({ path: location.pathname, label: "Vista detalle Reporte clínico" });
  } else if (location.pathname.includes("newReport")) {
    breadcrumbItems.push({ path: location.pathname, label: "Nuevo reporte de evolución clínica" });
  } else if (location.pathname.includes("EditReport")) {
    breadcrumbItems.push({ path: location.pathname, label: "Nuevo reporte clínico" });
  }
