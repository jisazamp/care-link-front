  ...
  useEffect(() => {
    if (isEditMode && evolution) {
      form.setFieldsValue({
        ...
        date: evolution.fecha_evolucion ? dayjs(evolution.fecha_evolucion) : undefined,
      ...
      <div style={{ maxWidth: 1214, margin: "0 auto 24px 0", paddingLeft: 32, paddingRight: 32 }}>
        <div className="breadcrumbs" style={{ fontSize: 14, color: "#8C8C8C", marginBottom: 24 }}>
          ...
          <span className="current" style={{ color: "#222", fontWeight: 500 }}>
            {isEditMode ? "Editar reporte de evolución" : "Nuevo reporte de evolución"}
          </span>
        </div>
        <h1 className="page-title" style={{ fontSize: 20, fontWeight: 500, color: "#222", margin: "0 0 12px 0" }}>
          {isEditMode ? "Editar reporte de evolución clínica" : "Nuevo reporte de evolución clínica"}
        </h1>
      </div>
      {/* Framer principal */}
      <Card className="main-frame" style={{ width: "100%", margin: "0 auto", borderRadius: 8, background: "#fff", padding: 0, border: "none" }}>
        ...
        <Card style={{ width: "100%", margin: "0 0 24px 0", borderRadius: 8, padding: "24px 32px" }} bordered={false}>
          ...
          <Row style={{ marginTop: 12 }}>
            <Col span={24}><Text strong>Observaciones:</Text> {report?.observaciones}</Col>
          </Row>
        </Card>
        ...
        <Card style={{ width: "100%", margin: "0 0 24px 0", borderRadius: 8, padding: "24px 32px" }} bordered={false}>
          ...
          <Divider style={{ margin: "16px 0" }} />
          <Form layout="vertical" form={form} onFinish={handleFinish} style={{ width: "100%" }}>
            <Row gutter={32}>
              <Col span={12}>
                <Form.Item label="Profesional" name="professional" rules={[{ required: true, message: "Campo requerido" }]}> 
                  <Select
                    ...
                    options={professionalsQuery.data?.data.data.map((p: any) => ({
            ...
            <Row gutter={32}>
              <Col span={12}>
                <Form.Item label="Fecha de registro" name="date" rules={[{ required: true, message: "Campo requerido" }]}> 
                  <DatePicker style={{ width: "100%" }} />
                </Form.Item>
              </Col>
              <Col span={24}>
                <Form.Item label="Observación" name="observation" rules={[{ required: true, message: "Campo requerido" }]}> 
            ...
            <Row justify="end" style={{ marginTop: 24 }}>
              ...
              <Button htmlType="submit" type="primary" loading={isEditMode ? editEvolution.isPending : createEvolution.isPending}>
