...
export const UserHomeVisitDetails: React.FC = () => {
  ...
  const {
    data: homeVisits,
    isLoading: loadingHomeVisits,
    refetch: refetchHomeVisits,
  } = useGetUserHomeVisits(userId);
      ...
      render: (_, record) => (
        <Space>
          ...
          <Button
            ...
            size="small"
            type="link"
            onClick={() => handleDeleteFamilyMember(record.id_acudiente)}
          >
            Eliminar
          </Button>
        </Space>
  ...
  const homeVisitsColumns: ColumnsType<HomeVisit> = [
    ...
    },
    {
      ...
      render: (estado: string, record: any) => {
        ...
        const updateDate = record.fecha_actualizacion
        ...
        return (
          <Tooltip
            title={
              isAutoCompleted
                ? `Actualizada automáticamente el ${updateDate?.toLocaleDateString()} a las ${updateDate?.toLocaleTimeString()}`
                : undefined
            }
          >
            <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
              <span
                style={{ color: config[estado as keyof typeof config]?.color }}
              >
          ...
          return (
            <Tooltip title={`Profesional asignado: ${profesional}`}>
              ...
              >
                ...
                />
                <span
                  style={{ fontWeight: 500, color: "#1890ff", cursor: "help" }}
                >
                  {profesional}
                </span>
              </div>
      ...
      render: (_, record) => (
        <Space>
          <Tooltip title="Ver detalles de la visita y editarla si es necesario">
            <Link
              to={`/visitas-domiciliarias/usuarios/${userId}/editar-visita/${record.id_visitadomiciliaria}`}
            >
              <Button type="link" className="main-button-link" size="small">
          ...
          >
            ...
            >
                ...
                <div className="user-details-card" style={{ width: "100%" }}>
                  <div className="user-details-head">
                    ...
                    <div className="user-details-actions">
                      <Link
                        to={`/visitas-domiciliarias/usuarios/${userId}/editar`}
                      >
                    ...
                    record?.data.data ? (
                      <Space>
                        <Button
                          ...
                          onClick={() =>
                            navigate(
                              `/visitas-domiciliarias/usuarios/${userId}/historia`,
                            )
                          }
                        >
                          Editar
              ...
              <Panel header="Reportes Clínicos" key="3">
                  ...
                  extra={
                    ...
                    >
                      <Button
                        ...
                        onClick={() =>
                          navigate(
                            `/visitas-domiciliarias/usuarios/${userId}/nuevo-reporte`,
                          )
                        }
                        className="main-button-white"
                ...
                >
                  {medicalReports?.data.data &&
                  medicalReports.data.data.length > 0 ? (
                    <Table
                        ...
                        },
                        {
                          ...
                          render: (_, record) =>
                            record.fecha_registro
                              ? dayjs(record.fecha_registro).format(
                                  "DD-MM-YYYY",
                                )
                              : "",
                        },
              ...
              <Panel header="Acudientes" key="4">
                <Card
                  extra={
                    <Link
                      to={`/visitas-domiciliarias/usuarios/${userId}/familiar`}
                    >
                      <Button
                        className="main-button-white"
                        variant="outlined"
              ...
              <Panel
                header={
                  ...
                  >
                    ...
                    {homeVisits?.data.data &&
                      homeVisits.data.data.length > 0 && (
                        <div
                          style={{
                            display: "flex",
                            alignItems: "center",
                        ...
                        >
                          <span>
                            ({homeVisits.data.data.length} visita
                            {homeVisits.data.data.length !== 1 ? "s" : ""})
                          </span>
                          {homeVisits.data.data.some(
                            (v: any) => v.profesional_asignado,
                          ) && (
                            <span style={{ color: "#52c41a" }}>
                              •{" "}
                              {
                                homeVisits.data.data.filter(
                                  (v: any) => v.profesional_asignado,
                                ).length
                              }{" "}
                          ...
                          {homeVisits.data.data.some(
                            (v: any) => v.estado_visita === "REALIZADA",
                          ) && (
                            <span style={{ color: "#52c41a" }}>
                              •{" "}
                              {
                    ...
                    >
                      Las visitas vencidas se marcan automáticamente como
                      "Realizadas". Al reprogramar, el estado cambia a
                      "Reprogramada".
                    </div>
                  </div>
                }
                ...
                >
                  {loadingHomeVisits ? (
                    ...
                    >
                      ...
                      />
                      <Typography.Text type="secondary">
                        No hay visitas domiciliarias programadas para este
                        paciente
                      </Typography.Text>
                      <Typography.Text
                        type="secondary"
