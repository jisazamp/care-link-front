...
import {
  useUpdateTransporte,
  useUpdateTransporteHorarios,
} from "../../hooks/useUpdateTransporte/useUpdateTransporte";
...
export const Transporte: React.FC = () => {
  ...
  const { mutate: updateTransporte, isPending: updateLoading } =
    useUpdateTransporte();
  const { mutate: updateHorarios, isPending: updateHorariosLoading } =
    useUpdateTransporteHorarios();
  ...
  const getTiempoRestante = (
    horaRecogida: string | null | undefined,
  ...
  const handleStartEditHorario = (
    record: RutaTransporte,
    field: "hora_recogida" | "hora_entrega",
  ) => {
    setEditingHorarios({
      ...
      },
      {
        onSuccess: () => {
          ...
          if (
            ...
            transporteSeleccionado.id_transporte === id_transporte
          ) {
          ...
          {alertasStats && (
            <Space size={screens.xs ? "small" : "middle"}>
              {alertasStats.urgentes > 0 && (
                <Badge
                  count={alertasStats.urgentes}
                  style={{ backgroundColor: "#ff4d4f" }}
                >
                  <Tag
                    color="red"
                    style={{ fontSize: screens.xs ? "12px" : "14px" }}
                    className="transporte-alert-tag"
                  >
          ...
          onRow={(record) => {
            ...
            return {
              ...
              style: {
                cursor: "pointer",
                backgroundColor:
                  alertaColor === "transparent"
                    ? "transparent"
                    : `${alertaColor}15`,
                borderLeft:
                  alertaColor !== "transparent"
                    ? `4px solid ${alertaColor}`
                    : "none",
                ...
                <Col span={24}>
                  ...
                  >
                    <div>
                      ...
                      <br />
                      <Text>
                        {transporteSeleccionado.direccion_recogida ||
                          "No especificada"}
                      </Text>
                    </div>
                    <div>
                      ...
                      <br />
                      <Text>
                        {transporteSeleccionado.telefono_contacto ||
                          "No especificado"}
                      </Text>
                    </div>
                  </Space>
            ...
            >
              <Row gutter={16}>
                <Col span={12}>
                  ...
                  {(() => {
                    const isEditing =
                      editingHorarios?.id_transporte ===
                        transporteSeleccionado.id_transporte &&
                      editingHorarios?.field === "hora_recogida";
                      ...
                      return (
                        <Space style={{ marginTop: "8px" }}>
                          <TimePicker
                            format="HH:mm:ss"
                            value={
                              editingHorarios.value
                                ? dayjs(editingHorarios.value, "HH:mm:ss")
                                : null
                            }
                            onChange={(time) => {
                              setEditingHorarios((prev) =>
                                prev
                                  ? {
                                      ...prev,
                                      value: time
                    ...
                    return (
                      <Space style={{ marginTop: "8px" }}>
                        <Text>
                          {transporteSeleccionado.hora_recogida ||
                            "No especificada"}
                        </Text>
                        <Button
                          type="text"
                ...
                <Col span={12}>
                  ...
                  {(() => {
                    const isEditing =
                      editingHorarios?.id_transporte ===
                        transporteSeleccionado.id_transporte &&
                      editingHorarios?.field === "hora_entrega";
                    ...
                    return (
                      <Space style={{ marginTop: "8px" }}>
                        <Text>
                          {transporteSeleccionado.hora_entrega ||
                            "No especificada"}
                        </Text>
                        <Button
                          type="text"
