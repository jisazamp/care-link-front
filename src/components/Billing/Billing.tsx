...
export const Billing: React.FC = () => {
  ...
  const { data: facturacionCompleta, isLoading: isLoadingFacturacionCompleta } =
    useGetFacturacionCompleta();
  ...
  const filteredBills = React.useMemo(() => {
    ...
    if (invoiceFilters.factura) {
      filtered = filtered.filter((bill: Bill) =>
        bill.numero_factura
          ?.toLowerCase()
          .includes(invoiceFilters.factura.toLowerCase()),
      );
    }

    if (invoiceFilters.estado) {
      filtered = filtered.filter(
        (bill: Bill) => bill.estado_factura === invoiceFilters.estado,
      );
    }
    ...
    if (filters.estado) {
      filtered = filtered.filter(
        (bill: Bill) => bill.estado_factura === filters.estado,
      );
    }

    if (filters.contrato) {
      filtered = filtered.filter(
        (bill: Bill) => bill.id_contrato?.toString() === filters.contrato,
      );
    }
  ...
  const stats = {
    ...
    pagadas: filteredBills.filter((b: Bill) => b.estado_factura === "PAGADA")
      .length,
    pendientes: filteredBills.filter(
      (b: Bill) => b.estado_factura === "PENDIENTE",
    ).length,
    vencidas: filteredBills.filter((b: Bill) => b.estado_factura === "VENCIDA")
      .length,
    totalValor: filteredBills.reduce(
      (acc: number, b: Bill) => acc + (Number(b.total_factura) || 0),
      0,
    ),
  ...
  const handleFormSubmit = (values: any) => {
    if (editingBill) {
      updateFactura.mutate(
        { id: editingBill.id_factura, data: values },
        {
          onSuccess: () => {
  ...
  const filteredFacturacionCompleta =
    facturacionCompleta?.filter((item: any) => {
      // Filtro por documento
      if (
        contractFilters.documento &&
        !item.n_documento
          ?.toLowerCase()
          .includes(contractFilters.documento.toLowerCase())
      ) {
        return false;
      }
      // Filtro por estado
      if (
        contractFilters.estado &&
        item.estado_contrato !== contractFilters.estado
      ) {
        return false;
      }
      // Filtro por contrato
      if (
        contractFilters.contrato &&
        !item.id_contrato?.toString().includes(contractFilters.contrato)
      ) {
        return false;
      }
      // Filtro por tipo contrato
      if (
        contractFilters.tipoContrato &&
        !item.tipo_contrato
          ?.toLowerCase()
          .includes(contractFilters.tipoContrato.toLowerCase())
      ) {
        return false;
      }
      ...
      if (
        contractFilters.facturaAsociada &&
        !item.numero_factura
          ?.toLowerCase()
          .includes(contractFilters.facturaAsociada.toLowerCase())
      ) {
        return false;
      }
    ...
    <div style={{ padding: "24px" }}>
      ...
      <Card title="Gestión de Contratos">
        <Table
          ...
          rowKey={(row) =>
            `${row.id_pago || ""}-${row.id_factura || ""}-${row.id_contrato || ""}-${row.id_usuario || ""}`
          }
          ...
          >
            ...
            {filteredBills.length === 0 && !isLoadingFacturas && (
              <div style={{ textAlign: "center", marginTop: 24 }}>
                ...
                <Typography.Text type="secondary">
                  Comience creando una nueva factura para gestionar los pagos de
                  los contratos.
                </Typography.Text>
                <br />
                <Button
