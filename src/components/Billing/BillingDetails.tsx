import { Descriptions, Divider, Empty, Table, Tag } from "antd";
...
export const BillingDetails: React.FC<BillingDetailsProps> = ({ bill }) => {
  ...
  const pagosColumns = [
    ...
    },
    {
      ...
      render: (v: string, r: any) => v || r.id_metodo_pago,
    },
    {
      title: "Tipo",
      dataIndex: "tipo_pago_nombre",
      key: "tipo_pago_nombre",
      render: (v: string, r: any) => v || r.id_tipo_pago,
    },
    {
      title: "Valor",
      dataIndex: "valor",
  ...
  return (
    <div>
      <Descriptions
        title={`Factura #${bill.numero_factura || bill.id_factura}`}
        bordered
        column={2}
      >
        ...
        <Descriptions.Item label="Impuestos">
          $ {Number(bill.impuestos || 0).toLocaleString()}
        </Descriptions.Item>
        <Descriptions.Item label="Descuentos">
          $ {Number(bill.descuentos || 0).toLocaleString()}
        </Descriptions.Item>
        <Descriptions.Item label="Total">
          $ {Number(bill.total_factura || 0).toLocaleString()}
        </Descriptions.Item>
        <Descriptions.Item label="Estado" span={2}>
          <Tag
            color={getEstadoColor(bill.estado_factura || "PENDIENTE")}
      ...
      {bill.pagos && bill.pagos.length > 0 ? (
        ...
        <Empty
          description="No hay pagos registrados para esta factura"
          style={{ margin: "24px 0" }}
        />
      )}
    </div>
