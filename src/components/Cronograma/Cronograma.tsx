...
export const Cronograma: React.FC = () => {
  ...
  const [selectedPacientes, setSelectedPacientes] = useState<
    PacientePorFecha[]
  >([]);
  const [justificacionModalVisible, setJustificacionModalVisible] =
    useState(false);
  const [selectedPaciente, setSelectedPaciente] =
    useState<CronogramaAsistenciaPaciente | null>(null);
  const [loadingAction, setLoadingAction] = useState(false);
  const [botonCargando, setBotonCargando] = useState<null | number | string>(
    null,
  );
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetCronogramasPorRango(startOfMonth, endOfMonth);
  ...
  useEffect(() => {
    ...
    if (highlightPatientId && shouldOpenModal && cronogramas?.data?.data) {
      ...
      for (const cronograma of cronogramas.data.data) {
        const paciente = cronograma.pacientes.find(
          (p) => p.id_cronograma_paciente === patientId,
        );
        if (paciente) {
      ...
      if (foundPatient && foundDate) {
        ...
        if (pacientes.length > 0) {
          setSelectedPacientes(
            pacientes.map((e) => e.paciente as unknown as PacientePorFecha),
          );
  ...
  const handleEstadoChange = (pacienteId: number, nuevoEstado: string) => {
    ...
    const paciente = selectedPacientes.find(
      (p) => p.id_cronograma_paciente === pacienteId,
    );
    if (paciente && paciente.estado_asistencia !== "PENDIENTE") {
      message.error(
        `No se puede cambiar el estado de un paciente con estado "${paciente.estado_asistencia}". Solo se puede modificar registros con estado "PENDIENTE".`,
      );
      return;
    }
      ...
      updateEstado(
        {
          ...
          data: {
            estado_asistencia: nuevoEstado as
              | "PENDIENTE"
              | "ASISTIO"
              | "NO_ASISTIO"
              | "CANCELADO",
        ...
        },
        {
          onSuccess: (response) => {
            ...
            setSelectedPacientes((prev) =>
              prev.map((p) =>
                p.id_cronograma_paciente ===
                updatedPaciente.id_cronograma_paciente
                  ? { ...p, ...updatedPaciente }
                  : p,
              ),
            );
  ...
  const handleJustificacionConfirm = (
    estado: string,
        ...
        onSuccess: (response) => {
          ...
          if (estado === "REAGENDADO") {
            notification.success({
              ...
              description:
                "El paciente fue reagendado con justificación. El día NO se descuenta de la tiquetera.",
            });
          }
          refetch();
        },
  ...
  const handleReagendar = (observaciones: string, nuevaFecha: string) => {
    ...
    if (selectedPaciente.estado_asistencia !== "PENDIENTE") {
      message.error(
        `No se puede reagendar un paciente con estado "${selectedPaciente.estado_asistencia}". Solo se puede reagendar pacientes con estado "PENDIENTE".`,
      );
      return;
    }
    setLoadingAction(true);
      ...
      },
      {
        onSuccess: () => {
          ...
          setSelectedPacientes((prev) =>
            prev.map((p) =>
              p.id_cronograma_paciente ===
              selectedPaciente.id_cronograma_paciente
                ? {
                    ...p,
                    estado_asistencia: "REAGENDADO" as any,
                    observaciones,
                  }
                : p,
            ),
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (estado: string) => (
        <Tag color={getEstadoColor(estado)}>{estado.replace("_", " ")}</Tag>
      ),
    },
    {
      title: "Acciones",
        ...
        return (
          <Space>
            <Button
              ...
              type="primary"
              onClick={() =>
                handleEstadoChange(record.id_cronograma_paciente, "ASISTIO")
              }
              disabled={isDisabled}
              loading={
                botonCargando === `${record.id_cronograma_paciente}-ASISTIO`
              }
              title={
                !isPending
                  ? 'Solo se pueden modificar registros con estado "PENDIENTE"'
                  : ""
              }
            >
              Asistió
            ...
            <Button
              ...
              danger
              onClick={() =>
                handleEstadoChange(record.id_cronograma_paciente, "NO_ASISTIO")
              }
              disabled={isDisabled}
              loading={
                botonCargando === `${record.id_cronograma_paciente}-NO_ASISTIO`
              }
              title={
                !isPending
                  ? 'Solo se pueden modificar registros con estado "PENDIENTE"'
