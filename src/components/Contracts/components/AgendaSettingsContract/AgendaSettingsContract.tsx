...
export const AgendaSettingsContract = ({ services, onBack }: AgendaSettingsContractProps) => {
  const [selectedDatesService, setSelectedDatesService] = useState<string[]>([]);
  const [selectedDatesTransport, setSelectedDatesTransport] = useState<string[]>([]);
  ...
  useEffect(() => {
    const selectedService = services.find((s) => s.serviceType.includes("Tiquetera"));
    const selectedTransport = services.find((s) => s.serviceType.includes("Transporte"));
  ...
  const handleSelectDate = (date: Dayjs, type: "service" | "transport") => {
    ...
    if (type === "service") {
      if (selectedDatesService.includes(formattedDate)) {
        setSelectedDatesService(selectedDatesService.filter((d) => d !== formattedDate));
      ...
      if (selectedDatesTransport.includes(formattedDate)) {
        setSelectedDatesTransport(selectedDatesTransport.filter((d) => d !== formattedDate));
        ...
        {maxServiceDays > 0 && (
          ...
          >
            <Calendar
              ...
              dateFullCellRender={(date) => renderDateCell(date, selectedDatesService)}
        ...
        {maxTransportDays > 0 && (
          ...
          >
            <Calendar
              ...
              dateFullCellRender={(date) => renderDateCell(date, selectedDatesTransport)}
              style={{ width: "300px", margin: "auto" }}
            />
          </Card>
        )}
