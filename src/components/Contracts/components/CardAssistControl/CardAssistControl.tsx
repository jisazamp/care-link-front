...
export const CardAsistControl = () => {
  ...
  const [selectedPatient, setSelectedPatient] =
    useState<AsistenciaDiaria | null>(null);
  ...
  const handleEstadoChange = (pacienteId: number, nuevoEstado: string) => {
    ...
    const paciente = attendanceData?.data.data?.find(
  ...
  const handleReagendar = (observaciones: string, nuevaFecha: string) => {
    ...
    if (selectedPatient.estado_asistencia !== "PENDIENTE") {
      message.error(
        `No se puede reagendar un paciente con estado "${selectedPatient.estado_asistencia}". Solo se puede reagendar pacientes con estado "PENDIENTE".`,
      );
      return;
    }
    setLoadingAction(true);
  ...
  const canReagendar =
    selectedPatient?.estado_asistencia === "PENDIENTE" &&
      ...
      title: (
        <Checkbox
          checked={
            ...
            selectedRows.length === (attendanceData?.data.data?.length || 0)
          }
          indeterminate={
            selectedRows.length > 0 &&
            selectedRows.length < (attendanceData?.data.data?.length || 0)
          ...
          onChange={(e) => {
            if (e.target.checked) {
              setSelectedRows(
                attendanceData?.data.data?.map(
                  (p) => p.id_cronograma_paciente,
                ) || [],
              );
            } else {
              setSelectedRows([]);
        ...
        <Checkbox
          ...
          onChange={(e) => {
            if (e.target.checked) {
              ...
              setSelectedRows(
                selectedRows.filter(
                  (id) => id !== record.id_cronograma_paciente,
                ),
              );
            }
          }}
            ...
            <Tooltip title="Ver en cronograma y resaltar paciente">
              <Button
                ...
                onClick={() => {
                  ...
                  navigate(
                    `/cronograma?highlightPatient=${record.id_cronograma_paciente}&openModal=true`,
                  );
                }}
              >
                Ver
    ...
    <>
      ...
      >
        <div style={{ marginBottom: 16 }}>
          <p>
            <strong>Paciente:</strong> {selectedPatient?.nombres}{" "}
        ...
        <Form layout="vertical">
          <Form.Item label="Observaciones / Justificación" name="observaciones">
            <Input.TextArea
              rows={4}
